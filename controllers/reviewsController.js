  ...
  try {
    ...
    if (invalidReviews.length > 0) {
      return res.status(400).json({
        ...
        error: {
          message:
            "Some reviews are missing required fields or contain invalid data.",
          invalidReviews,
        },
      });
    }
...
export const createReview = async (req, res, next) => {
  try {
    ...
    const businessExists = await Business.findById(business).populate(
      "userId",
      "email firstname"
    );
    if (!businessExists) {
...
const getReviewStats = async (business, user, filter) => {
  if (business && !user) {
...
const buildMatchStage = (user, business, search) => {
  const match = {};
  if (user) {
...
export const getAllReviews = async (req, res, next) => {
  try {
    ...
    const { totalReviews, averageRating } = await getReviewStats(
      business,
      user,
      filter
    );

    const matchStage = buildMatchStage(user, business, search);
